...
class RaytraceApplet : public IApplet {
public:
    RaytraceApplet();
    ~RaytraceApplet() override = default;
    void setup() override;
...
RaytraceApplet::RaytraceApplet() { }

void RaytraceApplet::setup() {
    // Initialize scene
...
bool RaytraceApplet::intersectSphere(const Ray& ray, const vmath_hpp::fvec3& center, float radius, float& t_hit) const {
...
bool RaytraceApplet::intersectPlane(const Ray& ray, const vmath_hpp::fvec3& normal, float d, float& t_hit) const {
...
vmath_hpp::fvec3 RaytraceApplet::getPlaneColor(const vmath_hpp::fvec3& point) const {
    float scale = 0.1f;
...
HitInfo RaytraceApplet::traceScene(const Ray& ray) const {
    HitInfo closestHit;
...
vmath_hpp::fvec3 RaytraceApplet::castRay(const Ray& ray, int depth) const {
...
void RaytraceApplet::loop(JaDraw<WIDTH, HEIGHT>& canvas, float dt, const InputData& inputs) {
    t += dt;
    ...
    #pragma omp parallel for schedule(dynamic) // Optional OpenMP parallelization
...
const char* RaytraceApplet::getName() const {
